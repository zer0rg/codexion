...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/12 17:43:13 by rgerman-          #+#    #+#             */
/*   Updated: 2026/05/12 17:43:14 by rgerman-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	try_acquire_dongles(t_coder *coder)
{
	...
	{
		if (!dongle_acquire(&coder->sim->dongles[first_id], coder->id,
				deadline))
			return (0);
...
void	*coder_routine(void *arg)
{
	...
	while (1)
	{
		...
		if (coder->sim->stop_simulation
			|| coder->compile_count >= coder->sim->compile_target)
		{
			pthread_mutex_unlock(&coder->sim->state_mutex);
			break ;
		}
