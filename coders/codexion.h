...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/12 17:43:36 by rgerman-          #+#    #+#             */
/*   Updated: 2026/05/12 17:58:59 by rgerman-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void						log_state(t_simulation *sim, int coder_id,
								const char *message);

t_priority_queue			*heap_create(int capacity);
void						heap_destroy(t_priority_queue *heap);
void						heap_insert(t_priority_queue *heap, int coder_id,
								long long priority, long long request_time);
int							heap_extract_min(t_priority_queue *heap,
								long long *priority);
...
void						heap_update_priority(t_priority_queue *heap,
								int coder_id, long long new_priority);
int							init_coders_threads(t_simulation *sim);
int							init_monitor_thread(t_simulation *sim,
								pthread_t *monitor);
...
int							simulation_init(t_simulation *sim, int argc,
								char **argv);
