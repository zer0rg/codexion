...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/12 17:43:08 by rgerman-          #+#    #+#             */
/*   Updated: 2026/05/12 17:59:16 by rgerman-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	validate_args(t_simulation *sim, int argc, char **argv)
{
	if (argc != 9)
	{
		fprintf(stderr,
				"Usage: \
			%s number_of_coders time_to_burnout time_to_compile time_to_debug \
...
int	simulation_run(t_simulation *sim)
{
	...
	i = 0;
	if (!init_coders_threads(sim))
		return (-1);
	if (!init_monitor_thread(sim, &monitor))
		return (-1);
